...
fromArray xs = unfold f zero where
  f n = pure $ Tuple (n + one) <$> (xs A.!! n)
...
take n fa | n < one = nil
          | otherwise = stepMap f fa where
  f (Yield a s) = Yield a s' where s' = take (n - one) <$> s
...
takeWhile f = stepMap g where
  g (Yield a s) | f a       = Yield a (takeWhile f <$> s)
                | otherwise = Done
...
drop n fa | n < one = fa
          | otherwise = stepMap f fa where
  f (Yield a s) = Skip s' where s' = drop (n - one) <$> s
